/**
 * CartX Setup Script — run: node setup.js
 * Creates missing directories and writes the inventory page.
 */
const fs   = require('fs')
...
export default function InventoryPage() {
  ...
  // Any logged-in user can edit/delete their own products
  function canModify(product) {
    if (!user) return false
    const owner = product.createdBy?._id || product.createdBy
    // If owner matches current user OR user is admin
    return String(owner) === String(user._id) || user.role === 'admin' || !owner
  }
  ...
  useEffect(() => {
    const loadProducts = async () => { await fetchPage(1, true) }
    loadProducts()
  }, [fetchPage])
        ...
        {loading ? (
          ...
          <>
            ...
            <div ref={sentinelRef} style={{ height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 12 }}>
              ...
              {!hasMore && products.length > 0 && <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>All products loaded</p>}
            </div>
          </>
        )}
      </div>
...
console.log('\n✅ CartX setup complete! Run: node setup.js && npm run dev')
