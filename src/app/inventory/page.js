...
export default function InventoryPage() {
  ...
  const fetchNext = useCallback(async (isReset) => {
    if (isFetching.current) return
    isFetching.current = true
    if (isReset) setLoading(true); else setLoadingMore(true)

    const params = { limit: 15, sortBy: 'createdAt', order: 'desc', ...(!isReset && cursorRef.current ? { cursor: cursorRef.current } : {}), ...(search && { search }) }
  ...
  // Re-run whenever sentinel mounts/unmounts (loading changes) or fetchNext changes
  useEffect(() => {
    if (!sentinelRef.current) return
    if (observerRef.current) observerRef.current.disconnect()
    observerRef.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && hasMoreRef.current && !isFetching.current) fetchNext(false)
    }, { threshold: 0.1 })
  ...
  const inp = {
    ...
    borderColor: 'var(--input-border)', borderRadius: 8,
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <div>
            ...
            <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: 4 }}>{total} products total</p>
          </div>
        ...
        {/* Sentinel always in DOM so observer can attach after initial load */}
        <div ref={sentinelRef} style={{ height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 12 }}>
          ...
          {!loading && !hasMore && products.length > 0 && <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>All products loaded</p>}
        </div>
      </div>
